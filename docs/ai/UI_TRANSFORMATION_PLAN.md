...
> **STATUS: ACTIVO — parcialmente aplicado.** Cimientos implementados en `lib/core/ui/` (FudiColors, FudiTypography, FudiSpacing, FudiRadius, theme light/dark con `NoSplash.splashFactory`). La theme migration de `lib/features/business/presentation/` está completa. **Pendiente**: features consumer (`lib/features/{landing,explore,offers,orders,home,favorites,profile,all_offers}/presentation/`) aún usan colores hardcodeados (`Colors.white`, `Colors.black*`, `Color(0x...)`).
...
- [ ] **Muteo Global del Splash:** Configurar `splashFactory: NoSplash.splashFactory` en `ThemeData` en [fudi_theme.dart](lib/core/ui/fudi_theme.dart).
...
- [ ] Actualizar los tokens en [fudi_spacing.dart](lib/core/ui/fudi_spacing.dart) bajo la clase existente `FudiRadius` en lugar de crear nuevas clases:
...
- [ ] **Mapear en `FudiTypography`:** Configurar [fudi_typography.dart](lib/core/ui/fudi_typography.dart) aplicando contrastes agresivos de peso (ej. `FontWeight.w800` para títulos y `FontWeight.w400` para cuerpo).

---

## 2. Paleta de Colores y Combinaciones Estratégicas

Basándonos en la inspección de los logos oficiales ([icon.svg](assets/svgs/icon.svg) y [role_wordmark.svg](assets/svgs/role_wordmark.svg)), unificaremos la paleta de colores en [fudi_colors.dart](lib/core/ui/fudi_colors.dart) y estableceremos combinaciones recomendadas para generar jerarquía.
...
- [ ] Modificar la decoración global en `inputDecorationTheme` dentro de `FudiTheme` ([fudi_theme.dart](lib/core/ui/fudi_theme.dart)):
...
- [ ] Rediseñar [fudi_surface_card.dart](lib/core/ui/fudi_surface_card.dart):
...
- [ ] Mantener [FudiStickyPageHeader](lib/core/ui/fudi_sticky_page_header.dart) como un `PreferredSizeWidget` basado en `AppBar` para pantallas utilitarias (ej. Ajustes, Métodos de Pago, Historial).
...
- [ ] Actualizar [fudi_colors.dart](lib/core/ui/fudi_colors.dart) y [fudi_spacing.dart](lib/core/ui/fudi_spacing.dart).
...
- [ ] Validar todos los componentes visualmente en la pantalla [ui_gallery_screen.dart](lib/core/ui/ui_gallery_screen.dart).
