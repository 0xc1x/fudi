...
Genera boilerplate de un feature según el patrón real del repo (mirror de `lib/features/offers`, `business`, `auth`): `data/supabase_<feature>_repository.dart`, `domain/<feature>.dart` + `<feature>_repository.dart`, `presentation/<feature>_providers.dart` + `<feature>_screen.dart`.

**Uso:**

```bash
python .ai/mcp/scripts/generate-flutter-feature.py <nombre_feature>
```
...
**Nota:** los screens se generan como `*_screen.dart` (no `*_page.dart`), los providers en `*_providers.dart` y la implementación en `supabase_*_repository.dart` — siguiendo las convenciones existentes del codebase.
...
**Supabase:** el MCP oficial es remoto/OAuth por harness (sin env var): `opencode mcp auth supabase`.
