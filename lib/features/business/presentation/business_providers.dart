...
/// Catalog providers
final businessOffersProvider = FutureProvider.family<List<Offer>, String>((ref, businessId) async {
  return ref.watch(businessCatalogRepositoryProvider).getBusinessOffers(businessId);
});

/// Order providers
final businessOrdersProvider = FutureProvider.family<List<OrderModel>, String>((ref, businessId) async {
  return ref.watch(businessOrderRepositoryProvider).getBusinessOrders(businessId);
});

final businessOrdersStreamProvider = StreamProvider.family<List<OrderModel>, String>((ref, businessId) {
  return ref.watch(businessOrderRepositoryProvider).watchBusinessOrders(businessId);
});

/// Stats providers
final businessStatsProvider = FutureProvider.family<BusinessStats, String>((ref, businessId) async {
  return ref.watch(businessStatsRepositoryProvider).getBusinessStats(businessId);
});
...
final selectedBusinessIdProvider = NotifierProvider<SelectedBusinessId, String?>(SelectedBusinessId.new);

/// All businesses owned by the current user
final userBusinessesProvider = FutureProvider<List<BusinessProfile>>((ref) async {
...
final currentBusinessProvider = FutureProvider<BusinessProfile?>((ref) async {
  ...
  if (selectedId != null) {
    return businesses.firstWhere((b) => b.id == selectedId, orElse: () => businesses.first);
  }
