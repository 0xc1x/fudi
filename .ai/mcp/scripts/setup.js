...
function checkNodeVersion() {
	try {
		...
		if (majorVersion < 18) {
			logError(
				`Node.js versión ${version} detectada. Se requiere Node.js 18 o superior.`,
			);
			return false;
		}
...
function verifyLaunchers() {
	...
	const requiredLaunchers = ["github.mjs"];
...
function verifyEnvFile() {
	...
	for (const envFilePath of envFiles) {
		if (fs.existsSync(envFilePath)) {
			const relativePath =
				path.relative(REPO_ROOT, envFilePath) || path.basename(envFilePath);
			...
			try {
				...
				logWarning(
					`   No se pudo leer ${path.relative(REPO_ROOT, envFilePath)}`,
				);
			}
		}
	}
...
# Supabase MCP (official, remote) — auth via OAuth per harness:
#   opencode: opencode mcp auth supabase
#   codex: codex mcp login supabase
# No SUPABASE_DB_URL needed.
...
function showNextSteps() {
	...
	log("   - Supabase: autenticar el MCP oficial por OAuth (por harness):");
	log(
		"     opencode mcp auth supabase   (o el comando de auth de tu herramienta)\n",
	);
